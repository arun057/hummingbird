...
ImpressionProvider.prototype.getCollection= function(sr, callback) {
  if( sr == "Test" ) {
	  this.db.collection('test-impressions', function(error, impression_collection) {
...
ImpressionProvider.prototype.save = function(impressions, callback) {
	if(impressions.AP == "Test"){
		this.getCollection("Test", function(error, impression_collection) {
	      if( error ) {
	        callback(error);
		...
		this.getCollection("impressions", function(error, impression_collection) {
	      if( error ) {
	        ...
	        for( var i =0;i< impressions.length;i++ ) {
	          impression = impressions[i];
	          impression.created_at = new Date().toJSON();
	        }
